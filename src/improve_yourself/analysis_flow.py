...
function draw(){summary.textContent=`${data.source.map_id} · ${data.roster.length} Spieler · ${data.scenes.length} zusammengeführte Szenen · ${mode==='full_demo'?'Full Demo':selected.size+' Spieler gewählt'}`;chosen.innerHTML=[...selected].map(id=>`<span class="pill">${byId.get(id)?.display_name||id}</span>`).join('');player.innerHTML='';for(const p of data.roster.filter(p=>!selected.has(p.player_id)))player.add(new Option(`${p.display_name} · ${p.teams.join('/')}`,p.player_id));const visible=data.scenes.filter(s=>mode==='full_demo'||s.player_ids.some(id=>selected.has(id)));scenes.innerHTML=visible.map(s=>`<div class="scene"><strong>Runde ${s.round_number} · ${s.anchor_types.join(', ')}</strong><div class="muted">Kontext ${s.start_tick}–${s.end_tick} · Marker ${s.marker_ticks.join(', ')}</div><code>${s.review.command}</code></div>`).join('')||'<p>Keine Szenen für diese Auswahl.</p>';cs2status.textContent='CS2 coordinator actions are unavailable in static local-review mode.'}
