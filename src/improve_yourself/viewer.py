...
const sceneEl=document.querySelector('#scene'),frameEl=document.querySelector('#frame'),playEl=document.querySelector('#play'),speedEl=document.querySelector('#speed'),eventEl=document.querySelector('#event-info'),sceneInfoEl=document.querySelector('#scene-info');let playing=false,timer=null,img=null,zoom=1;const ZOOM_MIN=.5,ZOOM_MAX=3,ZOOM_STEP=.25;
...
function setZoom(value){zoom=Math.min(ZOOM_MAX,Math.max(ZOOM_MIN,value));document.querySelector('#zoom-reset').textContent=Math.round(zoom*100)+' %';draw()}canvas.addEventListener('wheel',event=>{event.preventDefault();setZoom(zoom+(event.deltaY<0?ZOOM_STEP:-ZOOM_STEP))},{passive:false});document.querySelector('#zoom-in').onclick=()=>setZoom(zoom+ZOOM_STEP);document.querySelector('#zoom-out').onclick=()=>setZoom(zoom-ZOOM_STEP);document.querySelector('#zoom-reset').onclick=()=>setZoom(1);sceneEl.onchange=reset;frameEl.oninput=draw;document.querySelector('#previous-frame').onclick=()=>step(-1);document.querySelector('#next-frame').onclick=()=>step(1);document.querySelector('#previous-scene').onclick=()=>{sceneEl.value=Math.max(0,Number(sceneEl.value)-1);reset()};document.querySelector('#next-scene').onclick=()=>{sceneEl.value=Math.min(replay.scenes.length-1,Number(sceneEl.value)+1);reset()};playEl.onclick=()=>{playing=!playing;playEl.textContent=playing?'⏸ Pause':'▶ Abspielen';clearInterval(timer);if(playing)timer=setInterval(()=>{const max=Number(frameEl.max);frameEl.value=Number(frameEl.value)>=max?0:Number(frameEl.value)+1;draw()},100/Number(speedEl.value))};speedEl.onchange=()=>{if(playing){playEl.onclick() ;playEl.onclick()}};reset();</script></body></html>'''
