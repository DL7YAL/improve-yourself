...
def render_viewer_state(store:ReplayStore,context:ReplayContext,output_path:Path,*,background_path:Path|None=None)->Path:
    state=viewer_state(store,context)
    if state["frame"]["tick"]!=state["resolved_tick"]: raise ValueError("viewer state frame tick must equal resolved tick")
    meta=json.loads((_root()/"resources/map_overviews/maps"/(state["map_id"]+".json")).read_text())
    # Generated collision/map previews are tooling/debug evidence only. They are never
    # selected automatically as product backgrounds; an approved asset must be explicit.
    bg=background_path
    available=bg is not None and bg.is_file()
    model={"state":state,"transform":meta["transform"],"canvas":meta["canvas"],"background":_uri(bg if available else None),"background_kind":"local_override" if background_path else "product_map_unavailable","background_available":available}
    output_path.parent.mkdir(parents=True,exist_ok=True); output_path.write_text(_HTML.replace("__MODEL__",json.dumps(model,separators=(",",":")).replace("<","\\u003c")),encoding="utf-8"); return output_path
def render_viewer(replay_path:Path,output_path:Path,*,radar_path:Path|None=None,pos_x:float=0,pos_y:float=0,scale:float=1,scenes:list[dict[str,Any]]|None=None)->Path:
    ...
    if payload.get("schema")==REPLAY_SCHEMA:
        ...
        if scale<=0: raise ValueError("radar scale must be positive")
        frame=payload["scenes"][0]["frames"][0]; state={"schema":"iy.viewer_state/v1","map_id":payload["map_name"],"round_number":payload["scenes"][0]["round_number"],"requested_tick":frame["tick"],"resolved_tick":frame["tick"],"selected_player_id":None,"selected_player":{"player_id":None},"view_mode":"tactical_2d","frame":frame}; model={"state":state,"transform":{"origin_world":{"x":pos_x,"y":pos_y},"world_units_per_pixel":scale},"canvas":{"width":1024,"height":1024},"background":_uri(radar_path),"background_kind":"legacy","background_available":radar_path is not None and radar_path.is_file()}; output_path.parent.mkdir(parents=True,exist_ok=True); output_path.write_text(_HTML.replace("__MODEL__",json.dumps(model,separators=(",",":")).replace("<","\\u003c")),encoding="utf-8"); return output_path
    if payload.get("schema")!=REPLAY_V2_SCHEMA: raise ValueError(f"expected {REPLAY_SCHEMA} or {REPLAY_V2_SCHEMA} replay payload")
    store=ReplayStore(replay_path); controller=ReplayController(store); scene=(scenes or store.manifest.get("scenes",[])); scene=scene[0] if scene else None; context=controller.seek(scene["round_number"],scene.get("review_tick",scene.get("tick"))) if scene else controller.snapshot(); context=controller.select_player(scene.get("focus_player_id")) if scene and scene.get("focus_player_id") else context; return render_viewer_state(store,context,output_path,background_path=radar_path)
def main()->int:
...
_HTML='''<!doctype html><meta charset="utf-8"><title>Improve Viewer</title><style>body{margin:0;background:#08111d;color:#edf5ff;font:14px system-ui}.shell{max-width:1280px;margin:auto;padding:20px}.head,.panel{background:#101f31;border:1px solid #29435e;border-radius:14px;padding:16px}.grid{display:grid;grid-template-columns:1fr 240px;gap:16px;margin-top:16px}.stage{position:relative;aspect-ratio:1}.stage img{width:100%;height:100%}.m{position:absolute;width:14px;height:14px;border-radius:50%;transform:translate(-50%,-50%);background:#ff9d3f;border:2px solid white}.ct{background:#3b9bff}.sel{outline:4px solid #f5d66b}.dir{position:absolute;width:22px;height:2px;background:#fff;transform-origin:left}.player{padding:8px;margin:6px;background:#162b42;border-radius:7px}</style><main class=shell><header class=head><b>IMPROVE YOURSELF · TACTICAL VIEWER</b><div id=c></div></header><section class=grid><div class=panel><div id=s class=stage></div></div><aside class=panel><b>Spieler</b><div id=p></div></aside></section></main><script>const M=__MODEL__,S=M.state,T=M.transform,O=T.origin_world,sc=T.world_units_per_pixel,C=M.canvas,s=document.querySelector('#s');if(M.background_available){let i=new Image;i.src=M.background;s.append(i)}else{s.textContent='PRODUCT_MAP_UNAVAILABLE'};c.textContent=`${S.map_id} · Runde ${S.round_number} · Tick ${S.resolved_tick} · angefordert ${S.requested_tick}`;for(const x of S.frame.players.filter(x=>x.alive!==false)){let px=(x.x-O.x)/sc/C.width*100,py=(O.y-x.y)/sc/C.height*100,e=document.createElement('i');e.className='m '+(x.side==='CT'?'ct':'')+(x.player_id===S.selected_player_id?' sel':'');e.style.left=px+'%';e.style.top=py+'%';s.append(e);let d=document.createElement('i');d.className='dir';d.style.left=px+'%';d.style.top=py+'%';d.style.transform=`rotate(${-x.yaw}deg)`;s.append(d);let r=document.createElement('div');r.className='player';r.textContent=x.name+' · '+x.side;p.append(r)}</script>'''
