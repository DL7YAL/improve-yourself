...
<section class="card bar"><label>Szene <select id="scene"></select></label><button id="previous-scene">‹ Szene</button><button id="next-scene">Szene ›</button><button id="previous-frame">‹ Frame</button><button id="play">▶ Abspielen</button><button id="next-frame">Frame ›</button><label>Tempo <select id="speed"><option value="0.5">0,5×</option><option value="1" selected>1×</option><option value="2">2×</option></select></label><button id="zoom-out" aria-label="Herauszoomen">−</button><button id="zoom-reset">100 %</button><button id="zoom-in" aria-label="Hineinzoomen">+</button><input id="frame" type="range" min="0" value="0"><span id="tick" class="pill"></span></section>
<section class="card stage"><canvas id="canvas" width="1024" height="1024"></canvas></section>
<section class="card meta"><span class="pill">T = orange · CT = blau · Linie = Blickrichtung</span><span class="pill">Utility: Rauch = grau · Feuer = orange · Flash = weiß · HE = rot · Decoy = violett</span><span class="pill">Tote verschwinden nur ab einem dokumentierten Kill-Tick.</span><span id="notice" class="pill warn"></span></section><section class="card"><strong id="scene-info"></strong><div id="event-info" class="pill"></div></section>
...
const sceneEl=document.querySelector('#scene'),frameEl=document.querySelector('#frame'),playEl=document.querySelector('#play'),speedEl=document.querySelector('#speed'),eventEl=document.querySelector('#event-info'),sceneInfoEl=document.querySelector('#scene-info');let playing=false,timer=null,img=null,zoom=1,panX=0,panY=0,dragging=false,dragStart=null;const ZOOM_MIN=.5,ZOOM_MAX=3,ZOOM_STEP=.25;
...
function draw(){ctx.clearRect(0,0,1024,1024);ctx.save();ctx.translate(512+panX,512+panY);ctx.scale(zoom,zoom);ctx.translate(-512,-512);if(img)ctx.drawImage(img,0,0,1024,1024);else{ctx.strokeStyle='#1c3046';for(let n=0;n<=1024;n+=128){ctx.beginPath();ctx.moveTo(n,0);ctx.lineTo(n,1024);ctx.stroke();ctx.beginPath();ctx.moveTo(0,n);ctx.lineTo(n,1024);ctx.stroke()}}
 const f=frame();if(!f)return;const s=scene(),index=Number(frameEl.value),activePlayers=f.active_players||f.players,gap=Math.max(1,(s.end_tick-s.start_tick)/s.frames.length);document.querySelector('#tick').textContent=`Tick ${f.tick} · Frame ${index+1}/${s.frames.length}`;sceneInfoEl.textContent=`Runde ${s.round_number} · Multi-Kill: ${s.marker_player}`;const nearby=(s.events||[]).filter(e=>Math.abs(e.tick-f.tick)<=gap);const utilities=(s.utility||[]).filter(u=>u.area?(u.start_tick<=f.tick&&f.tick<=u.end_tick):(u.kind==='flash'||u.kind==='he'?Math.abs(f.tick-u.end_tick)<=gap:(u.start_tick<=f.tick&&f.tick<=u.end_tick)));eventEl.textContent=[...nearby.map(e=>`Kill: ${e.attacker} → ${e.victim} (${e.weapon})${e.marker_multikill?' · Multi-Kill':''}`),...utilities.map(u=>`Utility: ${u.kind} · ${u.thrower||'Werfer unbekannt'}`)].join(' | ')||'Kein Kill oder Utility-Ereignis an diesem Snapshot.';
 const utilityStyle={smoke:'#aeb7c4',fire:'#ff8c42',flash:'#fff',he:'#ff5b5b',decoy:'#c783ff'};for(const u of utilities){const path=(u.path||[]).map(p=>project(p,activePlayers));const endpoint=path[path.length-1]||(u.target&&project(u.target,activePlayers));if(path.length>1){ctx.strokeStyle=utilityStyle[u.kind]||'#fff';ctx.globalAlpha=.85;ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(...path[0]);path.slice(1).forEach(p=>ctx.lineTo(...p));ctx.stroke();}if(endpoint){const [x,y]=endpoint;ctx.fillStyle=utilityStyle[u.kind]||'#fff';ctx.globalAlpha=u.area?.32:.9;ctx.beginPath();ctx.arc(x,y,u.area?34:15,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;}}
 for(const p of activePlayers){const [x,y]=project(p,activePlayers),color=p.side.toUpperCase()==='CT'?'#55aaff':'#ff9f43',a=p.yaw*Math.PI/180;ctx.strokeStyle=color;ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(x,y);ctx.lineTo(x+Math.cos(a)*34,y-Math.sin(a)*34);ctx.stroke();ctx.fillStyle=color;ctx.beginPath();ctx.arc(x,y,10,0,Math.PI*2);ctx.fill();ctx.font='16px system-ui';ctx.fillStyle='#fff';ctx.fillText(p.name,x+14,y-12)}ctx.restore()}
...
function clampPan(){const limit=Math.max(0,(zoom-1)*512);panX=Math.min(limit,Math.max(-limit,panX));panY=Math.min(limit,Math.max(-limit,panY))}function resetView(){zoom=1;panX=0;panY=0;document.querySelector('#zoom-reset').textContent='100 %';draw()}function setZoom(value){zoom=Math.min(ZOOM_MAX,Math.max(ZOOM_MIN,value));clampPan();document.querySelector('#zoom-reset').textContent=Math.round(zoom*100)+' %';draw()}canvas.addEventListener('wheel',event=>{event.preventDefault();setZoom(zoom+(event.deltaY<0?ZOOM_STEP:-ZOOM_STEP))},{passive:false});canvas.addEventListener('pointerdown',event=>{if(zoom<=1)return;dragging=true;dragStart={x:event.clientX,y:event.clientY,panX,panY};canvas.setPointerCapture(event.pointerId);canvas.style.cursor='grabbing'});canvas.addEventListener('pointermove',event=>{if(!dragging||!dragStart)return;const rect=canvas.getBoundingClientRect();panX=dragStart.panX+(event.clientX-dragStart.x)*canvas.width/rect.width;panY=dragStart.panY+(event.clientY-dragStart.y)*canvas.height/rect.height;clampPan();draw()});function endPan(event){if(!dragging)return;dragging=false;dragStart=null;if(canvas.hasPointerCapture(event.pointerId))canvas.releasePointerCapture(event.pointerId);canvas.style.cursor='grab'}canvas.addEventListener('pointerup',endPan);canvas.addEventListener('pointercancel',endPan);canvas.style.cursor='grab';document.querySelector('#zoom-in').onclick=()=>setZoom(zoom+ZOOM_STEP);document.querySelector('#zoom-out').onclick=()=>setZoom(zoom-ZOOM_STEP);document.querySelector('#zoom-reset').onclick=resetView;sceneEl.onchange=reset;frameEl.oninput=draw;document.querySelector('#previous-frame').onclick=()=>step(-1);document.querySelector('#next-frame').onclick=()=>step(1);document.querySelector('#previous-scene').onclick=()=>{sceneEl.value=Math.max(0,Number(sceneEl.value)-1);reset()};document.querySelector('#next-scene').onclick=()=>{sceneEl.value=Math.min(replay.scenes.length-1,Number(sceneEl.value)+1);reset()};playEl.onclick=()=>{playing=!playing;playEl.textContent=playing?'⏸ Pause':'▶ Abspielen';clearInterval(timer);if(playing)timer=setInterval(()=>{const max=Number(frameEl.max);frameEl.value=Number(frameEl.value)>=max?0:Number(frameEl.value)+1;draw()},100/Number(speedEl.value))};speedEl.onchange=()=>{if(playing){playEl.onclick() ;playEl.onclick()}};reset();</script></body></html>'''
