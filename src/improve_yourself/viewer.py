...
def render_viewer_state(store:ReplayStore,context:ReplayContext,output_path:Path,*,background_path:Path|None=None)->Path:
    ...
    meta=json.loads((_root()/"resources/map_overviews/maps"/(state["map_id"]+".json")).read_text()); bg=background_path or _root()/"resources/generated_overviews"/state["map_id"]/"overview.svg"; available=bg.is_file()
    model={"state":state,"transform":meta["transform"],"canvas":meta["canvas"],"background":_uri(bg if available else None),"background_kind":"local_override" if background_path else "improve_generated","background_available":available}
