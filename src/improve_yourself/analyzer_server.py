...
def render_preflight(preflight_path: Path, output_path: Path) -> Path:
    ...
    players = "".join(f"<tr><td>{_text(p['name'])}</td><td>{_text(p['side'])}</td><td>{p['kills']}</td><td>{p['deaths']}</td><td>{p['headshots']}</td></tr>" for p in match["players"])
    ...
    document = f'''<!doctype html><html lang="de"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Improve Yourself – Analyse-Preflight</title><style>:root{{color-scheme:dark;font-family:system-ui;background:#08111e;color:#edf3fb}}body{{margin:0;padding:24px}}main{{max-width:1100px;margin:auto}}.card{{background:#111f32;border:1px solid #273a54;border-radius:12px;padding:16px;margin-top:16px}}table{{width:100%;border-collapse:collapse}}td,th{{padding:8px;text-align:left;border-bottom:1px solid #273a54}}button{{background:#58a6ff;color:#07111e;border:0;border-radius:9px;padding:12px 16px;font-weight:800;cursor:pointer}}.muted{{color:#aebfd3}}.warn{{background:#211c12;border-color:#8b6b2d}}</style><main><header><h1>Improve Yourself · Analyzer V1</h1><p class="muted">1 Profil → 2 Demo-Preflight → 3 Analyse → 4 Review</p></header><section class="card"><h2>1. Aktives Analyseprofil</h2><p><strong>{_text(profile['label'])}</strong> · Version {_text(profile['version'])}</p><p class="muted">{_text(profile['disclaimer'])}</p></section><section class="card"><h2>2. Demo geladen · technischer Preflight</h2><p><strong>Map:</strong> {_text(match['map_name'])} · <strong>Reguläre Runden:</strong> {match['regular_rounds']} · <strong>Endstand:</strong> {scores}</p><h3>Overall-Scoreboard</h3><table><thead><tr><th>Spieler</th><th>Seite</th><th>Kills</th><th>Tode</th><th>Headshots</th></tr></thead><tbody>{players}</tbody></table></section><section class="card warn"><h2>Datenqualität und Kriterien vor der Analyse</h2><p><strong>Vorhanden:</strong> {available}</p><p><strong>Nicht verfügbar:</strong> {missing}</p><ul>{quality}</ul><p>Fehlende Daten sind kein negativer Befund und werden nicht geschätzt.</p></section><section class="card"><h2>3. Analyse</h2><p>Die Analyse verwendet ausschließlich die oben als bewertbar ausgewiesenen Profilkriterien. Rohdaten und Fakten werden nicht verändert.</p><button id="start">Analyse starten</button> <span id="status" class="muted"></span></section></main><script>document.querySelector('#start').onclick=async()=>{{const b=document.querySelector('#start'),s=document.querySelector('#status');b.disabled=true;s.textContent='Analysiert lokal …';try{{const r=await fetch('/api/start-analysis',{{method:'POST'}}),d=await r.json();if(!r.ok)throw Error(d.error);location.href=d.review_url;}}catch(e){{s.textContent='Analyse fehlgeschlagen: '+e.message;b.disabled=false;}}}};</script></html>'''
