...
def _render_preflight_v1(data: dict[str, Any], output_path: Path) -> Path:
    ...
    document = document.replace('href="/assets/improve-yourself-mark.svg"', 'href="/assets/improve-yourself-logo-v3.png"')
    document = document.replace('<title>Improve Yourself · Preview V1</title>', '<title>Improve Yourself · Preview V1.1</title>')
    enhancements = '''
...
'''
    ...
    document = document.replace('<button data-tab="system">System Check</button>', '')
    document = document.replace("const tabs=['analyzer','system','optimizer','settings']", "const tabs=['analyzer','replay','optimizer','settings']")
    document = document.replace("</script>", enhancements + "</script>")
    v11_shell = '''
const optimizer=document.querySelector('#optimizer'),systemSection=document.querySelector('#system');
if(systemSection){systemSection.classList.remove('hidden');systemSection.removeAttribute('id');systemSection.querySelector('h1').textContent='System Check';optimizer.prepend(systemSection);}
const boards=document.querySelector('#analyzer .grid');if(boards){const children=[...boards.children];for(let i=0;i<children.length;i+=2){const panel=document.createElement('section');panel.className='team-panel';panel.append(children[i],children[i+1]);boards.append(panel);}}
const profileCard=document.querySelector('#analyzer .card:nth-of-type(2)');if(profileCard){const assessable=[...profileCard.querySelectorAll('.assessable')].length;profileCard.insertAdjacentHTML('beforeend',`<p class="criterion-count"><strong>Aktive Kriterien: ${assessable} / 5</strong><br><small>Nur fachlich implementierte Kriterien werden aktiv bewertet. Andere Kriterienfamilien sind sichtbar, aber nicht auswählbar, solange keine validierte Berechnung vorliegt.</small></p>`);}
'''
    document = document.replace("</script>", v11_shell + "</script>")
    wide_styles = '''<style>
.app{width:100%;max-width:none;margin:0;padding:clamp(14px,2vw,34px)}.top{width:100%;justify-content:space-between;gap:18px}.top .brand{flex:0 0 auto}.nav{justify-content:flex-end}.nav button{white-space:nowrap}main{max-width:none;width:100%;margin:22px 0}.card{padding:clamp(18px,2vw,30px)}#analyzer .grid{grid-template-columns:repeat(2,minmax(320px,1fr));align-items:start}.team-panel{min-width:0;background:#0b1727;border:1px solid #264766;border-radius:11px;padding:18px}.team-panel h3{margin-top:0}#system-result{display:grid;grid-template-columns:repeat(auto-fit,minmax(290px,1fr));gap:14px}.optimizer-grid{display:grid;grid-template-columns:minmax(0,1.15fr) minmax(320px,.85fr);gap:18px;margin-top:18px}.optimizer-panel{background:#0b1727;border:1px solid #264766;border-radius:11px;padding:18px}.optimizer-panel h2{margin-bottom:14px}.optimizer-panel .recommendation{margin:8px 0}.optimizer-status{display:inline-block;background:#19334d;border-radius:999px;padding:4px 8px;margin:0 0 10px;color:#dcefff;font-size:.9rem}.criterion-count{border-top:1px solid #264766;padding-top:12px}@media(max-width:900px){.app{padding:14px}.top{align-items:flex-start}.nav{justify-content:flex-start}.optimizer-grid,#analyzer .grid{grid-template-columns:1fr}}@media(max-width:620px){.top{flex-direction:column}.nav{width:100%}.nav button{flex:1 1 145px}.card{padding:16px}}
</style>'''
    wide_script = '''
const escapeHtml=value=>String(value??'—').replace(/[&<>'"]/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[char]));
renderSystem=function(data){const checks=data.checks||[];const byId=Object.fromEntries(checks.map(c=>[c.id,c]));const sourceLink=(url,label='Offizielle Herstellerseite')=>typeof url==='string'&&url.startsWith('https://')?`<a href="${escapeHtml(url)}" target="_blank" rel="noreferrer">${label}</a>`:'Offizieller Vergleich nicht verfügbar.';const driverDetail=id=>{const c=byId[id],e=c?.evidence||{};if(!c)return'';const installed=e.installed_package_version||e.installed_version||'Nicht zuverlässig erfasst';const official=e.official_version||'Nicht zuverlässig erfasst';return `<p><strong>Installiert:</strong> ${escapeHtml(installed)}<br><strong>Offizieller Stand:</strong> ${escapeHtml(official)}<br><strong>Bewertung:</strong> ${escapeHtml(c.user_view?.status||'Nicht prüfbar / unbekannt')}<br><small>${sourceLink(e.official_source)}</small></p>`};const cards=checks.map(c=>`<article class="recommendation"><strong>${escapeHtml(c.label)}</strong><br>${escapeHtml(c.summary||'Nicht prüfbar / unbekannt')}<br><small>${escapeHtml(c.user_view?.action||'Keine automatische Änderung.')}</small>${c.id==='gpu_driver'||c.id==='chipset_driver'?driverDetail(c.id):''}</article>`).join('');document.querySelector('#system-result').innerHTML=cards;const statusCards=checks.map(c=>`<article class="recommendation"><strong>${escapeHtml(c.label)}</strong><br><small>${escapeHtml(c.user_view?.status||'Nicht prüfbar / unbekannt')}</small></article>`).join('');const recommendationCards=checks.map(c=>`<article class="recommendation"><strong>${escapeHtml(c.label)}</strong><br><small>${escapeHtml(c.user_view?.action||'Noch nicht bewertet / für Preview V1 noch nicht definiert.')}</small></article>`).join('');const bios=byId.motherboard?.evidence||{};const biosNote=`<p class="muted"><strong>BIOS/UEFI:</strong> ${escapeHtml(bios.bios_version||'Nicht zuverlässig erfasst')}. Improve bewertet oder ändert BIOS/UEFI in Preview V1 nicht; eine Aktualisierung ist nur bei einem konkreten Herstellergrund manuell zu prüfen.</p>`;document.querySelector('#optimizer-result').innerHTML=`<div class="optimizer-grid"><section class="optimizer-panel"><h2>Mein System</h2>${statusCards}</section><section class="optimizer-panel"><h2>Empfehlungen / Bewertung</h2><span class="optimizer-status">Read-only Preview</span>${driverDetail('gpu_driver')}${driverDetail('chipset_driver')}${recommendationCards}${biosNote}<p class="muted">Performance und Quality unterscheiden nur vorhandene, belastbare Bewertungen. Es wird keine Einstellung angewendet.</p></section></div>`};
'''
...
def create_server(output_root: Path, port: int, *, max_mib: int = 2048, max_frames: int = 256, radar_path: Path | None = None, pos_x: float = 0, pos_y: float = 0, scale: float = 1) -> AnalyzerServer:
    ...
    shutil.copyfile(ASSET_ROOT / "improve-yourself-logo-v3.png", assets / "improve-yourself-logo-v3.png")
    shutil.copyfile(ASSET_ROOT / "improve-yourself-wordmark-v3.png", assets / "improve-yourself-wordmark-v3.png")
...
def _write_empty_analyzer(output_path: Path) -> None:
    output_path.write_text(f'''<!doctype html><meta charset="utf-8"><link rel="icon" href="/assets/improve-yourself-logo-v3.png"><title>Improve Yourself · Preview V1.1</title><style>:root{{color-scheme:dark;font-family:system-ui;background:#07111e;color:#edf7ff}}*{{box-sizing:border-box}}body{{margin:0}}.app{{width:100%;padding:clamp(14px,2vw,34px)}}.top{{display:flex;gap:18px;align-items:center;border-bottom:1px solid #264766;padding-bottom:14px}}.brand img{{width:270px;max-height:72px;object-fit:contain}}.nav{{margin-left:auto;display:flex;gap:8px;flex-wrap:nowrap}}button{{background:#19334d;color:#dcefff;border:1px solid #386384;border-radius:8px;padding:10px 12px;font-weight:800;cursor:pointer;white-space:nowrap}}button.primary{{background:#8edbff;color:#06101a;border:0}}button:disabled{{opacity:.45;cursor:not-allowed}}.card{{background:#102033;border:1px solid #264766;border-radius:14px;padding:clamp(20px,2vw,32px);margin-top:22px;box-shadow:0 8px 28px #0004}}.muted{{color:#a9c7dc}}@media(max-width:900px){{.top{{align-items:flex-start;flex-wrap:wrap}}.nav{{margin-left:0;flex-wrap:wrap}}}}@media(max-width:620px){{.app{{padding:14px}}.brand img{{width:210px}}.nav button{{flex:1 1 145px}}}}</style><main class="app"><header class="top">{brand_markup()}<nav class="nav"><button>Analyzer</button><button>Tactical Replay</button><button>Optimizer</button><button disabled title="In Entwicklung">My Improvement · In Entwicklung</button><button disabled title="In Entwicklung">Benchmark · In Entwicklung</button><button>Einstellungen</button></nav></header><section class="card"><h1>Analyzer</h1><p class="muted">Wähle eine lokale .dem, .dem.zst oder .dem.bz2-Datei. Es gibt keinen Upload und keine Cloud-Verbindung.</p><button id="choose" class="primary">Demo öffnen</button><p id="status" class="muted"></p></section><section class="card"><h2>Tactical Replay</h2><p class="muted">Gesamtes Match und Review-Szenen sind nach der Analyse verfügbar.</p></section></main><script>document.querySelector('#choose').onclick=async()=>{{const s=document.querySelector('#status');if(!window.pywebview?.api){{s.textContent='Diese Preview wird über Improve Yourself.exe geöffnet.';return}};s.textContent='Öffnet lokale Dateiauswahl …';try{{const r=await window.pywebview.api.choose_demo();if(r?.ok)location.href='/analyzer.html';else s.textContent=r?.error||'Keine Demo ausgewählt.'}}catch(e){{s.textContent='Demoauswahl fehlgeschlagen: '+e.message}}}};</script>''', encoding="utf-8")
