...
const reviewProfile = analysis.review_profile || { id: "Nicht verfügbar", version: "—", label: "Kein Bewertungsprofil im Analyseartefakt", disclaimer: "Keine Profilbewertung verfügbar." };
const hintsByScene = new Map((analysis.review_hints || []).map((hint) => [`${hint.observed_facts?.round_number}|${hint.observed_facts?.player}|${hint.observed_facts?.first_tick}`, hint]));
...
overview.getRange("F10:H10").merge(); overview.getRange("F10").values = [[`Aktives Bewertungsprofil: ${reviewProfile.label} (${reviewProfile.version})`]]; overview.getRange("F10:H10").format = { fill: palette.sky, wrapText: true, borders: { preset: "outside", style: "thin", color: palette.border } };
...
title(sceneSheet, "Multi-Kills & Szenen · Review-Einstieg", "A1:J1");
sceneSheet.getRange("A2:J2").values = [["Runde", "Spieler", "Kills", "Erster Tick", "Letzter Tick", "Beteiligte Opfer", "Bewertungskategorie", "Auslösendes Kriterium", "Schwelle", "Beobachtete Fakten"]];
sceneSheet.getRange(`A3:J${scenes.length + 2}`).values = scenes.map((scene) => { const hint=hintsByScene.get(`${scene.round_number}|${scene.player}|${scene.first_tick}`); return [scene.round_number, scene.player, scene.kill_count, scene.first_tick, scene.last_tick, scene.victims.join(", "), hint?.category || "vorhandener Marker", hint?.criterion_label || "Kein Profilhinweis", hint?.threshold ? `${hint.threshold.field} ${hint.threshold.operator} ${hint.threshold.value}` : "—", hint?.message || "Vorhandener Multi-Kill-Marker; im Spielkontext prüfen."]; });
header(sceneSheet, "A2:J2"); body(sceneSheet, `A3:J${scenes.length + 2}`); sceneSheet.getRange(`A3:E${scenes.length + 2}`).format.numberFormat = "#,##0";
...
infoSheet.getRange("A3:B10").values = [["Analyzer-Schema", analysis.schema], ["Report-Version", "iy.analyzer_excel_report/v1"], ["Bewertungsprofil", reviewProfile.label], ["Profil-/Kriterienversion", `${reviewProfile.id}/${reviewProfile.version}`], ["Erstellt", generatedAt], ["Map", analysis.map_name || "Unbekannt"], ["Quell-ID", sourceId ? `${sourceId}…` : "Nicht verfügbar"], ["Originaldemo", "Nicht eingebettet; kein lokaler Pfad enthalten"]];
infoSheet.getRange("A3:B10").format = { borders: { preset: "all", style: "thin", color: palette.border }, wrapText: true, verticalAlignment: "top" }; infoSheet.getRange("A3:A10").format.font = { bold: true };
section(infoSheet, "Interpretationshinweise", "A12:F12");
infoSheet.getRange("A13:F17").merge();
infoSheet.getRange("A13").values = [[`${reviewProfile.disclaimer} Dieser Bericht fasst vorhandene Analyzer-Ereignisse zusammen. Multi-Kill-Szenen und andere Hinweise dienen als Einstieg für eine menschliche Prüfung. Fehlende Datenquellen werden offen ausgewiesen und dürfen nicht als negativer Befund ausgelegt werden.`]];
infoSheet.getRange("A13:F17").format = { fill: palette.amber, wrapText: true, verticalAlignment: "top", borders: { preset: "outside", style: "thin", color: palette.border } };
