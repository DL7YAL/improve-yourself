...
$icon = Join-Path $root 'src\improve_yourself\assets\improve-yourself-logo-v3.ico'
if (!(Test-Path -LiteralPath $python)) { throw "Expected project virtual environment: $python" }
if (!(Test-Path -LiteralPath $icon)) { throw "Preview icon not found: $icon" }
