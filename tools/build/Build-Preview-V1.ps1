...
& $python -m PyInstaller --noconfirm --clean --onedir --name 'Improve Yourself' --paths (Join-Path $root 'src') --collect-all awpy --collect-all zstandard --add-data "$resources;resources\maps" --add-data "$(Join-Path $root 'src\improve_yourself\assets');improve_yourself\assets" --distpath $output --workpath (Join-Path $root 'build\pyinstaller') --specpath (Join-Path $root 'build\spec') (Join-Path $root 'src\improve_yourself\preview_launcher.py')
