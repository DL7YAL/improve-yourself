...
$cliNames = @('iy-analyze', 'iy-system-check', 'iy-benchmark-result', 'iy-optimizer-input', 'iy-workflow', 'iy-replay-viewer', 'iy-review-server', 'iy-analysis-flow', 'iy-demo-workflow', 'iy-analyzer-shell')
...
try {
    ...
    $version = & $venvPython -c "import sys; print('%d.%d' % (sys.version_info.major, sys.version_info.minor))"
    if ($LASTEXITCODE -ne 0) {
