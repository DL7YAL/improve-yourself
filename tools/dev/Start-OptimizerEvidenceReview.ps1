...
$systemArgs = @('--output', (Join-Path $out 'system-check.json'))
if ($Offline) { $systemArgs += '--offline' }
& $system @systemArgs; if($LASTEXITCODE -ne 0){throw "System Check failed: $LASTEXITCODE"}
