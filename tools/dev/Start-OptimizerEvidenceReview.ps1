[CmdletBinding()]
param([Parameter()][string]$OutputRoot = 'results\optimizer-evidence-review')
...
Write-Host '==> Collect local read-only System Check evidence' -ForegroundColor Cyan
& $system '--output' (Join-Path $out 'system-check.json'); if($LASTEXITCODE -ne 0){throw "System Check failed: $LASTEXITCODE"}
