...
| Benchmark Multi-Map-Transitions | Codex / The Beast | REVIEW | Der V1.2-Kandidat erhält reproduzierbar dokumentierte Valve-Runtime-Referenzen für Nuke Outside, Ancient B und Inferno Apps sowie fünf passgebundene Capture-Marker. Ein frischer CS2-Tools-Lauf am 2026-09-07 absolvierte Warmup und Messpass vollständig bis Event 60; alle Szenenreports und Marker erschienen in der festgelegten Reihenfolge. Die Controller-Evidenz trennt jetzt `runtime_status=complete` von `measurement_status=unverified`, weil Workshop-Filtering die angeforderten clientseitigen FPS-/Frametime-/`vprof`-Befehle abwies. Darauf aufbauend existiert eine eigenständige lokale Ergebnisstrecke mit fail-closed Capture-/Controller-Validierung, persönlichen profilgebundenen Top 10 und einer screenshot-tauglichen Offline-HTML-Seite. Sie enthält ausdrücklich keine Optimizer-, Account-, Upload-, Discord-, Steam- oder öffentliche Leaderboard-Anbindung. Ein normaler Viewer-Capture für die tatsächliche Landmark-Sichtbarkeit und ein nachweislich aktiver Messsammler bleiben offen. Der frühere VRAD-Preflight-/Full-Compile-Blocker bleibt als separate SDK-Evidenz gültig; der aktuelle VMAP-Quellstand war bereits danach kompiliert und wurde in diesem Controller-/Runtime-Slice nicht neu gebaut. |
...
| Improve Yourself – Experimental | Codex / The Beast | REVIEW | **Home: FUNCTIONALLY ACCEPTED / VISUAL FINALIZATION DEFERRED.** Home bleibt unverändert. Optimizer Foundation und der neue Network Quality Collector V1 sind read-only: deklarierte Ziele werden lokal als RTT/Jitter/Packet-Loss-Evidenz gemessen, ohne eine Recommendation oder Netzwerkkonfiguration abzuleiten. Keine Apply-Autorität, Systemänderung oder Benchmark-Aussage. |
| UI Visual Authority | Tristan | LOCKED | Canva `DAHUBn5D2aM` (`Versuch.nr1`), Revision 25, ist seit 2026-09-05 die alleinige visuelle Quelle. Alle früheren PDF-, UI-Pack- und Optimizer-MASTER sind `SUPERSEDED` und dürfen nicht erneut implementiert werden. Runtime-Code bleibt bis zum gesonderten Implementierungsauftrag unverändert. |
| 2D Analyzer / Viewer | Codex / The Beast | REVIEW | Der bestehende V2-Viewer bleibt kanonisch. Szenen-/Frame-Navigation, Tempoauswahl und evidenzgebundene Death-Visibility sind V2-konform integriert. AWPy-Rundenqualität, read-only Optimizer-Input und die getrennte V2-Review-Präsentation sind auf `codex/integrate-main-dev-v1` konsolidiert; PR #7 wartet auf den finalen technischen PR-Check. |
...
## Nächste Schritte
1. Branch-Integration: `codex/integrate-main-dev-v1` enthält den aktuellen `origin/main`-Stand per kontrolliertem Merge. `iy.replay/v2`, AnalyzerCore, AnalyzerDataHub sowie Optimizer Foundation/Evidence bleiben verbindlich; Azure und der Excel-Report bleiben getrennte Infrastruktur-/Exportpfade außerhalb des lokalen Produktkerns. Nächster Schritt ist ausschließlich der finale technische PR-Check von PR #7, kein Merge nach `main` ohne ausdrückliche Freigabe.
2. Benchmark-Abnahme und Szenenbau: den Folgeauftrag `docs/BENCHMARK_MAP_ASSET_REBUILD_WORK_ORDER.md` ausführen. Zuerst den bereits kompilierten V1.2-Kandidaten im normalen Viewer an den fünf `pass=measured`-Markern aufnehmen; danach nur belegte Lücken in der Reihenfolge Nuke Outside → Ancient B → Inferno Apps schließen und jeden Slice separat kompilieren und sichtbar abnehmen. Anschließend einen ausdrücklich gestarteten lokalen Messsammler auf das normalisierte Capture-Format abbilden und den ersten echten Lauf durch die eigenständige lokale Ergebnisstrecke prüfen. Performancewerte erst freigeben, wenn der Sammler seinen aktiven Zustand und verwertbare FPS-/Frametime-Daten belegt. Keine Optimizer-, Cloud-, Account-, Discord- oder öffentliche Leaderboard-Kopplung ergänzen. Der ältere VRAD-Preflight-/Full-Compile-Blocker bleibt für einen späteren VMAP-Neubau separat offen; keine Treiber-, Registry-, Adrenalin- oder Sicherheitsfilter-Änderung daraus ableiten.
