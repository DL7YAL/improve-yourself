...
| Benchmark Multi-Map-Transitions | Codex / The Beast | BLOCKED | Die Transformursache ist behoben: Das Authoring-Tool bäckt die Zielabmessungen jetzt in die Mesh-Vertices statt sich auf beim Compile verlorene Objekt-Skalierung zu verlassen. 32/32 Tests und Full Compile (`22 compiled, 0 failed, 1 skipped`) bestehen; der reale Lauf zeigt erstmals geschlossene Ancient-/Inferno-Korridorgeometrie und den ersten Smoke innerhalb dieser Geometrie. Die Gesamtsequenz ist noch nicht visuell freigegeben, weil Wasser, Red Room und insbesondere die fünf Inferno-Stufen in den Stichproben nicht eindeutig komponiert/erkennbar waren. Nächster Schritt ist ein marker-synchronisierter Kamera-/Landmark-Check auf der nun nachgewiesenen Laufzeitgeometrie; Smoke und Grundgeometrie nicht erneut blind verändern. |
...
## Nächste Schritte
1. Benchmark-Multi-Map-Transitions nicht als fertig behandeln: Die kompilierten Zielkorridore erscheinen jetzt an den Controller-Koordinaten. Als Nächstes Wasser/Red Room/Inferno-Stufen marker-synchronisiert erfassen und nur die erforderliche Kamerakomposition korrigieren; danach Nuke-Smoke -> Ancient B/Wasser -> Red Room -> Flash -> Inferno Apps vollständig visuell prüfen.
