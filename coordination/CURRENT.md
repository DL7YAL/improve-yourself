...
| Preview V1 Windows Packaging | Codex / The Beast | BLOCKED — Desktop-host final gate | Der neue native Desktop-Ansatz ist implementiert: `Improve Yourself.exe` startet eine eingebettete WebView2-Oberfläche mit ausschließlich dynamischem `127.0.0.1`-Backend, nativer Demoauswahl und sauberer Server-Lifecycle-Logik. Source-/Onedir-Start, eingebettete WebView2 und Loopback-Preflight wurden belegt; 57/57 Tests sind PASS. Ein frischer Onedir sowie ein Windows-standardkonformes ZIP wurden erstellt und ohne private Demo-/Output-/Repositoryinhalte geprüft. Der unbeaufsichtigte EXE-Test aus der frischen Entpackung beendet die Hostinstanz jedoch vor einem abrufbaren Listener, obwohl er den lokalen Datenordner anlegt. Daher ausdrücklich **nicht versandbereit**; keine neue Weitergabeempfehlung. Nächster Schritt: den Desktop-Host in einer interaktiven Windows-Sitzung gezielt stabilisieren und erst danach den vollständigen EXE-E2E- und Versandgate wiederholen. |
