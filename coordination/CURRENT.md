...
| 3D Viewer / World / Assets | Codex / The Beast | WAITING_FOR_TRISTAN (SLICE_C_ASSET_GATE) | Maschinenlesbares `iy.map_asset/v1`-Gate und 6 Tests sind vorhanden. Awpy-Anubis `.tri` (808.000 Triangles) ist koordinaten-/LOS-fähig, aber Build `17595823` gegen installiert `24828357` und damit nicht aktuell verifiziert; Valve-VPK ist current/hashbar, aber weder extrahiert noch zur Produktdistribution freigegeben. Entscheidung: current-build local-only, dokumentiert distributables Derivat oder 3D Anubis unavailable. Kein Renderer vor bestandenem Gate. |
