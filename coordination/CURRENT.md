...
| UI Visual Authority | Tristan | LOCKED | Canva `DAHUBn5D2aM` (`Versuch.nr1`), Revision 25, ist seit 2026-09-05 die alleinige visuelle Quelle. Alle früheren PDF-, UI-Pack- und Optimizer-MASTER sind `SUPERSEDED`. Die Canva-R25-Anwendungsshell wurde über PR #34 in `main` integriert; eine aktuelle sichtbare Gesamtprodukt-Abnahme bleibt offen. |
| 2D Analyzer / Viewer | Codex / The Beast | REVIEW | Der bestehende V2-Viewer bleibt kanonisch. Szenen-/Frame-Navigation, Tempoauswahl und evidenzgebundene Death-Visibility sind V2-konform integriert. PR #7 wurde am 2026-08-26 in `main` gemergt; der dortige finale technische PR-Check ist abgeschlossen. |
...
## Nächste Schritte
1. Aktuellen GitHub-`main` mit der integrierten Canva-R25-Shell sichtbar als Gesamtprodukt abnehmen: Startverhalten, Dashboard, Analyzer, Review, 2D Tactical Replay, My Improvement, Settings, read-only Optimizer sowie Empty-/Unknown-/Fehlerzustände. Keine Produktänderung aus der Abnahme ableiten, bevor deren Ergebnis belegt und separat autorisiert ist.
...
3. Vor weiterer Integration die vorhandenen schmutzigen Spezial-Worktrees einzeln read-only klassifizieren; nichts automatisch zusammenführen, bereinigen oder verwerfen. 3D-Viewer/World/Assets bleiben eine getrennte, nicht-benchmarkbezogene Vorbereitung.
