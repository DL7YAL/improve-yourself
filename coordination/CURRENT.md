...
| 3D Viewer / World / Assets | Codex / The Beast | SMOKE_EVIDENCE_GATE_DONE | Sightline-Auswertung/-Präsentation und feste Kameras sind bestanden. Smoke darf nur bei `utility_lifetimes=full` über eine offengelegte 144-Source-Unit-Kugelnäherung `clear/blocked` liefern; `partial/unavailable` und inkonsistente aktive Daten bleiben `unknown`. Realer Anubis-Stand ist partial und bleibt korrekt unknown; 91 Tests grün. NEXT: nur ReplayController→Renderer-Sessionadapter für kanonischen Frame, Seek, Spieler und FP/TP; keine neue UI/State Authority/Utility-Darstellung. |
