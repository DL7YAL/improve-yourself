...
| Demo Analyzer / lokale Desktop-Shell | Codex / The Beast | DONE | Dünner Tk-Adapter über der einzigen `iy-demo-workflow`-/Replay-v2-Wahrheit: reale Demo-Auswahl, Roster/Player Select ohne Reparse und fail-closed “In CS2 öffnen”. Das neue Readiness-Panel weist lokale Verbindung, aktiven `[DEMO]`-Modus und exakten Dateinamen getrennt aus; Review bleibt bis zum vollständigen PASS gesperrt und prüft beim Öffnen erneut. Realer PASS für `iy_ancient.dem`, korrekter FAIL für abweichenden erwarteten Namen. 104 Tests/8 CLIs und sichtbarer Panel-Smoke PASS. NEXT: explizit ein vorhandenes validiertes Workflow-Manifest wieder öffnen, ohne Ordnerscan oder Reparse. |
