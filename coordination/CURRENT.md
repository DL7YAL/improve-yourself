...
| Benchmark Multi-Map-Transitions | Codex / The Beast | WAITING_FOR_TRISTAN (geparkt) | `check_raytracing_support.vrad3` ist ein lokales Workshop-Tools/Hammer-Laufzeitartefakt, das erst nach Hammer-Initialisierung geprüft wird. Es wird weder in GitHub kopiert noch von Azure, Foundry, Cloud-CI oder MCP ausgewertet. Bei ausdrücklich wiederaufgenommener Benchmark-Arbeit nur den lokalen Hammer-/VRAD-Nachweis erfassen; keine Treiber-, Registry-, Adrenalin-, Smoke-, Geometrie- oder Controller-Änderung ohne neuen Auftrag. |
...
2. Tristan: `WAITING_FOR_TRISTAN` für eine ausdrückliche Wiederaufnahme der Benchmark-Arbeit. Nach Hammer-Start den lokalen VRAD-Preflight ausführen und den nicht-sensitiven Nachweis im Handoff erfassen. `check_raytracing_support.vrad3` bleibt eine lokale Workshop-Tools-Abhängigkeit und wird von Azure, Foundry, Cloud-CI und MCP ignoriert; keine Treiber-, Registry-, Adrenalin-, Smoke-, Geometrie- oder Controller-Änderung ohne neuen Auftrag.
