...
| Demo Analyzer / reproduzierbare Entwicklungsbaseline | Codex / The Beast | DONE | `tools/dev/Setup-V1.ps1` stellt Python 3.13 aus dem Lockfile her, prüft Abhängigkeiten, 45 Tests und sechs öffentliche CLI-Einstiegspunkte reproduzierbar. |
...
| System Check / Analyzer Output V1 | Codex / The Beast | WAITING_FOR_TRISTAN — System Check / Analyzer UX review | System Check ist jetzt fachlich vom Analyzer-/2D-/3D-Pfad getrennt: `iy-system-check` erzeugt explizite read-only Evidenz; `iy-optimizer-input` projiziert sie als versionierten, reinen Optimizer-Planungsinput ohne Apply/Restore. `iy-workflow` erzeugt nur Analyse/Replay/Viewer/Review und startet keinen System Check mehr. Bestehende System-Check-Funktionalität, Chipsatz-/GPU-/Anti-Cheat-Evidenz und Tests bleiben erhalten. |
