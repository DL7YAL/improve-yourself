...
| 3D Viewer / World / Assets | Codex / The Beast | SLICE_D_OBSTRUCTION_DONE | `ReplayRenderer`, deterministische FP/feste TP-Kamera und Native-Embed-Spike sind bestanden. Die feste TP-Kamera nutzt jetzt das hashgeprüfte local-only Visibility-Mesh: `clear/blocked/unknown`, letzte Segmentkollision, ausschließlich kollineare Anpassung plus 8 Source Units und `camera_adjusted`; reale Anubis-Kontrollpaare sowie 75 Tests bestanden. Assets bleiben ignoriert/nicht distributabel. NEXT: nur kanonischen `SightlineResult`-Evaluator über ReplayFrame + dieselbe Geometrie implementieren; noch kein Overlay/Smoke-Raten/Modelle/Packaging. |
