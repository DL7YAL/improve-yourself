...
| Benchmark Multi-Map-Transitions | Codex / The Beast | WAITING_FOR_TRISTAN | Steam-Dateiprüfung für CS2 (730) und SDK (745) ist abgeschlossen; Workshop Tools und Hammer starten wieder. Die Benchmark-VMAP lädt. Full Compile endet vor der eigentlichen Build-Pipeline am Hammer-Dialog `GPU Lightmap Baking`. Der vollständige read-only Preflight beweist für die RX 7900 XTX nicht nur DX12 Ultimate/FL 12_2, sondern auch den vom Hammer-Preflight verwendeten Vulkan-RT-Stack (`VK_KHR_acceleration_structure`, `VK_KHR_ray_tracing_pipeline`, `VK_KHR_ray_query`). Hammer ruft dafür fest `vrad3.exe -script check_raytracing_support.vrad3 -vulkan -gpuraytracing` auf, verwirft ihn aber ohne Detail-Log. Keine Smoke-, Geometrie-, Controller-, Treiber-, Registry-, Adrenalin- oder Hammer-/CS2-Konfigurationsänderung wurde vorgenommen. Tristan entscheidet über einen isolierten Preflight-Ausführungsversuch; danach Full Compile und marker-synchronisierten Kameramesslauf fortsetzen. |
...
## Nächste Schritte
1. Tristan: `WAITING_FOR_TRISTAN` für einen einzigen isolierten, temporären Preflight-Ausführungsversuch: mit bereits geöffnetem Workshop Tools exakt `vrad3.exe -script check_raytracing_support.vrad3 -vulkan -gpuraytracing` ausführen und ausschließlich Exitcode/Console-Ausgabe erfassen; danach Tools schließen und keine Konfiguration beibehalten. Lokale Evidenz bestätigt RX 7900 XTX, DirectX 12 Ultimate/FL 12_2/WDDM 3.2 und für eben Vulkan die RT-Erweiterungen Acceleration Structure, Ray-Tracing Pipeline und Ray Query. Der aktuelle Hammer-Dialog liefert keine genauere Begründung. Keine Treiber-, Registry-, Adrenalin-, Smoke-, Geometrie- oder Controller-Änderung ist vorgeschlagen. Erst bei einem klaren Ergebnis einen offiziellen Baking-/Treiber-Setup-Schritt beurteilen. Danach Full Compile und marker-synchronisierten Kameraabgleich fortsetzen.
2. Den vertex-gebakten Authoring-Stand und die neue Steam-/Hammer- sowie Full-Compile-Blocker-Evidenz sichern; keine erneute blinde Smoke- oder Grundgeometrieänderung.
