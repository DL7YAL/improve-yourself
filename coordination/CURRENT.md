...
| Analyzer Default Criteria / Filter V1 | Codex / The Beast | WAITING_FOR_TRISTAN — review | Neuer, ungemergter Branch `beast/analyzer-default-criteria-v1`: versioniertes Profil `iy.default_review/v1` bewertet ausschließlich vorhandene Fakten. Aktuell löst `Rundenweiter Multi-Kill` bei `kill_count >= 3` transparente Review-Hinweise aus; Profil, Kriterienversion, Kategorie, Kriterium, Schwelle und Fakten sind im realen Excel sichtbar/filterbar. Sichtlinie, Sound, Calls, Aim, Timing und Gegnerperspektive bleiben bei fehlender belastbarer Quelle ausdrücklich unbewertet. |
...
5. Tristan: `WAITING_FOR_TRISTAN — Analyzer default criteria / filter review`. Den frischen lokalen Beispielreport prüfen: Welche Fakten lösten die 22 Hinweise nach dem aktiven Improve-Defaultprofil aus und sind Profil-/Kriterienversion sowie Schwelle verständlich? Kein Merge vor dieser Abnahme.
