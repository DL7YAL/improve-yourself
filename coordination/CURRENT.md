...
| Preview V1 Windows Packaging | Codex / The Beast | IN_PROGRESS — Host-Fix-Regression | Die Console-Diagnose des gepackten Hosts belegt die Ursache: PyWebView enumeriert öffentliche `js_api`-Attribute rekursiv. Die öffentliche Bridge-Referenz auf das native Fenster führte dadurch in `window.native`, löste WebView2-Zugriffe außerhalb des UI-Threads (`CoreWebView2 can only be accessed from the UI thread`, COM `E_NOINTERFACE`) und eine Rekursionsüberläufe aus. Der kleinste Fix kapselt nur die Server-/Fensterreferenzen als private Bridge-Zustände; die einzige öffentliche API bleibt `choose_demo`. `58` Tests, Compile und `git diff --check` sind PASS. Ein frischer normaler Onedir-Testbuild bleibt nach acht Sekunden als antwortender nativer Host mit eigenem Loopback-Listener aktiv. Jetzt folgen neuer ZIP-/Entpacklauf und der vollständige Gate erneut ab Kaltstart 1/10; kein finaler Versand- oder Merge-Status ist daraus abgeleitet. |
