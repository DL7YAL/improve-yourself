...
| 3D Viewer / World / Assets | Codex / The Beast | SLICE_D_DONE | Backend-neutraler `ReplayRenderer`, deterministische FP-/feste TP-Kamera und Panda3D-native-Embed-Spike sind mit dem ignorierten local-only Anubis-Bundle bestanden: reales GLB geladen, kanonischer Tick/Spieler angewendet, Resize und Dispose bewiesen; 71 Tests grün. Rendererwahl bleibt offen, Assets bleiben ignoriert/nicht distributabel. NEXT: ausschließlich Visibility-Mesh-Abfrage und feste TP-Kamera-Obstruction-Anpassung mit `camera_adjusted`; noch keine Overlays/Smokes/Modelle/Verpackung. |
