...
| Preview V1 Windows Packaging | Codex / The Beast | BLOCKED | Reproduzierbarer PyInstaller-6.17.0-Onedir-Build und isolierter EXE-Smoke sind vorhanden: Loopback-Preflight/API/Review sowie read-only `iy.system_check/v1` → `iy.optimizer_input/v1` PASS. Versand bleibt blockiert, weil der bestehende Excel-Generator `@oai/artifact-tool` aus einer Entwicklungs-Node-Laufzeit benötigt und nicht durch die Distribution bereitgestellt wird. Kein ZIP als vollständige Preview V1 erzeugen, bis der Excel-Export paketierbar ist. |
