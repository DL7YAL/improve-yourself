...
| 3D Viewer / World / Assets | Codex / The Beast | SLICE_B_CONTROLLER_DONE | Der einzige mutable `ReplayController` konsumiert `iy.replay/v2`; Seek/Scrub, Szenen-/Eventnavigation, Player-/View-State und beobachtbare atomare Snapshots sind getestet. Die reale Anubis-Rohdemo enthält keine belegte Tickrate, daher bleibt Echtzeit-Play explizit gesperrt statt 64 Hz zu erfinden. NEXT: bestehende 2D-Ansicht auf Controller/v2 migrieren, zunächst mit deterministischer Navigation und sichtbarer Timing-Limitierung; noch kein 3D-Renderer. |
