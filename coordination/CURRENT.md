...
| 2D Tactical Replay Viewer V1 | Codex / The Beast | MERGE_BLOCKED — private lokale Demo-Referenz bereinigen | Technischer/funktionaler No-Merge-Review gegen `origin/main` `de087d9` ist PASS: 35/35 Tests, keine Secrets/Binaries/Ergebnisartefakte, konfliktfreie Merge-Simulation, Tristan-PASS korrekt dokumentiert und Main-Änderung ist unabhängig. Der Branch-Diff enthält jedoch in einem Handoff einen konkreten privaten lokalen Demo-Pfad samt Dateiname. Vor dem Merge muss diese Referenz durch eine nicht-private Beschreibung (z. B. nur Hash/Map/Aggregate) ersetzt und der kurze Review erneut geprüft werden. |
