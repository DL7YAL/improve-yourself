...
| Preview V1 Windows Packaging | Codex / The Beast | IN_PROGRESS — Desktop-host + wide UI gate | Der Root Cause des vorigen Kaltstartfehlers ist korrigiert: PyWebViews WinForms-Backend kehrt nach dem Erzeugen seines STA-UI-Threads aus `start()` zurück; der lokale Server bleibt deshalb jetzt bis zum nativen `closed`-Ereignis aktiv. Die verbindliche Wide-Desktop-Gridlogik nutzt die Fensterbreite, mit responsive Umbruch und Optimizer-Spalten „Mein System“ / „Empfehlungen / Bewertung“, weiterhin ohne Apply oder erfundene Regeln. 57/57 Tests, `pip check` und `git diff --check` sind PASS. Ein sichtbarer nativer HostFix-Build bleibt aktuell für die manuelle Desktopprüfung geöffnet; keine weitere UI-Eingabe durch Codex bis Tristan die Kontrolle freigibt. Danach: 10/10 Kaltstart-, sichtbarer Layout- und vollständiger EXE-E2E-Gate vor neuem ZIP. |
