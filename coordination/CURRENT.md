...
| 2D Analyzer / Viewer | Codex / The Beast | REVIEW | Der bestehende V2-Viewer bleibt kanonisch. Die aus `main` stammenden Viewer-V1-Funktionen werden ausschließlich als V2-konforme Navigation, Tempoauswahl und evidenzgebundene Death-Visibility integriert; die technische Schlussprüfung des Integrationsbranches läuft. |
...
## Nächste Schritte
1. Branch-Integration: `codex/integrate-main-dev-v1` führt `origin/main` kontrolliert in den aktuellen Dev-Stand. `iy.replay/v2`, AnalyzerCore und AnalyzerDataHub bleiben verbindlich; Main-Viewer-V1-Code wird nicht übernommen. Nach vollständiger technischer und Privacy-Validierung folgt ausschließlich Review, kein Merge nach `main` ohne ausdrückliche Freigabe.
