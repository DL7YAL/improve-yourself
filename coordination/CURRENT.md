...
| 3D Viewer / World / Assets | Codex / The Beast | SIGHTLINE_EVALUATOR_DONE | Renderer, feste Kameras und Obstruction sind bestanden. `SightlineResult` wird jetzt außerhalb des Renderers aus genau einem kanonischen Frame, exakten Tick/Spieler-IDs und derselben verifizierten Geometrie erzeugt; reale Anubis-Tick-6401-Paare liefern korrekt `clear+smoke unknown→unknown` und `blocked→occluded`. 81 Tests grün, Assets lokal/ignoriert. NEXT: nur Präsentationsgrenze für bereits evaluierte Sichtlinien und feste Farben; keine Geometrie-/Smoke-Neuberechnung oder Utility-Overlays. |
