...
| Benchmark Multi-Map-Transitions | Codex / The Beast | BLOCKED | Ein reproduzierbares Authoring-Tool legt nun projekt-eigene Ancient-/Inferno-Graybox-Korridore, Wasser-/Reflexionsfläche, Red-Room-Endpunkt, fünf Inferno-Stufen, Dächer und erweiterte Probe-Bounds in der VMAP an; der Controller verwendet deren gültige Koordinaten. 32/32 Tests und Full Compile bestehen. Der reale Laufzeitcheck bleibt FAIL: Das Kamerabild zeigt weiterhin den beige-grauen Ausgangsbereich statt der neuen Korridore. Nächster Schritt ist die Transform-/Instanzierungsursache zwischen kompilierten Meshes und Controller-Koordinaten anhand der kompilierten Laufzeitpositionen zu klären; keine weitere Smoke-, Fade- oder Blind-Geometrieänderung. |
...
## Nächste Schritte
1. Benchmark-Multi-Map-Transitions nicht als fertig behandeln: Die Zielgeometrie und gültigen Quellkoordinaten sind jetzt vorhanden, erscheinen im realen Lauf aber nicht an der erwarteten Kameraposition. Als Nächstes die tatsächlich kompilierten Mesh-Transforms/Instanzen gegen die Controllerpositionen messen; danach Nuke-Smoke -> Ancient B/Wasser -> Red Room -> Flash -> Inferno Apps erneut visuell prüfen.
2. Die aktuelle FAIL-Evidenz und den reproduzierbaren Authoring-Stand sichern; keine weiteren Smoke-/Fade-/Yaw- oder Blind-Geometrieänderungen vor der Transformdiagnose.
