...
OPEN: DONE is not claimed. With explicit user approval, the same hash-bound Mirage demo was copied unchanged to the local CS2 game directory as `game/csgo/iy_review.dem` because its original filename contains shell-sensitive characters. CS2 loaded it successfully and exposed the real ten-player line-up/events. Direct startup `demo_gototick 6352` was processed before demo readiness and stayed at the beginning. A second controlled run used CS2's `demo_pauseatservertick 6352`; playback advanced normally, then CS2 disconnected with the visible runtime error `Failed to parse message.` instead of proving a stable landing. Therefore tick 6352 is not runtime-accepted evidence yet. During the attempt Toggle Console was temporarily rebound from `^` to `F6` because the automation API cannot emit the German physical caret key; F6 did not open the console and the original binding has not been restored automatically. CS2 remains open on the error dialog for user inspection.
NEXT: First determine whether `Failed to parse message.` is intrinsic to this older demo at/before tick 6352 or specific to the pause-at-server-tick startup command, using the visible CS2 session/manual console if the user chooses. Restore Toggle Console from temporary `F6` to the user's original `^`. If ordinary playback also fails before the scene, obtain another real CS2 demo that is runtime-compatible with the installed client, run the unchanged workflow on it, and validate its generated scene tick. Do not mark DONE or alter scene rules to bypass this runtime incompatibility.
MODEL_PROFILE: terra
